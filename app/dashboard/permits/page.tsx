...
export default function PermitsPage() {
  ...
  const fetchPermits = async (page: number = 1, limit: number = DEFAULT_LIMIT) => {
    try {
      setIsLoading(true);
  ...
  const handleReindex = async () => {
    try {
      ...
      const response = await axiospermit.post("http://127.0.0.1:8000/api/rag/reindex");
      alert(response.data.message || "Reindexing completed successfully");
    } catch (error: any) {
      console.error("Error during reindexing:", error);
      alert("Reindexing failed: " + (error?.response?.data?.message || error.message));
    } finally {
  ...
  const runScrape = async () => {
    try {
      ...
      alert("Scrape failed: " + (error?.response?.data?.message || error.message));
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <Card className="mb-6">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1">Scrape City</label>
              <Input
                type="text"
                value={scrapeCity}
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Scrape Mode</label>
              <Select
                value={dateRange}
                onValueChange={(value) => {
                  setDateRange(value);
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            <div>
              <label className="block text-sm font-medium mb-1">Scheduler Hour</label>
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Scheduler Minute</label>
              <Select value={schedulerMinute} onValueChange={setSchedulerMinute}>
                <SelectTrigger>
                  <SelectValue placeholder="00" />
                </SelectTrigger>
                <SelectContent>
            ...
            <table className="w-full text-sm border rounded-lg">
              <thead className="bg-gray-100 text-xs font-medium uppercase">
                <tr className="border-b">
                  ...
                  <th className="text-left p-3">Work Class</th>
                  <th className="text-left p-3">Description</th>
                  <th className="text-left p-3">Status</th>
                  <th className="text-left p-3">Applicant</th>
                  <th className="text-left p-3">Applicant Address</th>
                  <th className="text-left p-3">Contractor</th>
                  <th className="text-left p-3">Company</th>
                  <th className="text-left p-3">Contractor Address</th>
              ...
              <tbody>
                {isLoading ? (
                  <tr>
                    <td colSpan={15} className="h-24 text-center text-gray-500">
                      Loading permits...
                    </td>
                  </tr>
                  ...
                  apiData.permits.map((permit, index) => (
                    <tr key={index} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3">{permit.permit_class_mapped || "N/A"}</td>
                      <td className="p-3">{permit.work_class || "N/A"}</td>
                      ...
                      <td className="p-3">{permit.applicant_name || "N/A"}</td>
                      <td className="p-3">{permit.applicant_address || "N/A"}</td>
                      <td className="p-3">{permit.contractor_name || "N/A"}</td>
                      <td className="p-3">{permit.contractor_company_name || "N/A"}</td>
                      <td className="p-3">{permit.contractor_address || "N/A"}</td>
                      <td className="p-3">{permit.contractor_phone || "N/A"}</td>
                      <td className="p-3">{permit.valuation || "N/A"}</td>
                      <td className="p-3">{permit.total_fees || "N/A"}</td>
